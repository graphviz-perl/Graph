...
attribute, that comes naturally in C<uniqedged> flavour. It does what
it sounds like, to unique or not the vertices participating in edges -
for example, is the hyperedge C<qw(a b a)> the same as the hyperedge
C<qw(a b)>. Without too much explanation:
...
L<Directed Hypergraphs and Applications|https://static.aminer.org/pdf/PDF/000/836/884/directed_hypergraphs_and_applications.pdf>.
...
This means turning off the C<uniq> property on a graph is especially
unsupported. Additionally, how the C<hyper> property of edges works may
